<?php
class Nepali_Calendar extends CI_Controller {
...
private function _get_nepali_month($m) {
    ...
    function convert() {
        ...
        if (isset($_POST['cnv'])) {
            ...
            switch ($_POST['cnv']) {
                case 'to_english':
                    $cal = $this->nep_to_eng($_POST['yy'], $_POST['mm'], $_POST['dd']);
                    //print_r($cal);
                    $english_conversion = $cal['year'] . '-' . $cal['month'] . '-' . $cal['date'] . ' ( ' . $cal['nmonth'] . '/' . $cal['day'] . ' )';
                    print_r($english_conversion);
                    break;
                case 'to_nepali':
                    //die('y');
                    $cal = $this->eng_to_nep($_POST['yy'], $_POST['mm'], $_POST['dd']);
                    //print_r($cal);
                    $nepali_conversion = $cal['year'] . '-' . $cal['month'] . '-' . $cal['date'] . ' ( ' . $cal['nmonth'] . '/' . $cal['day'] . ' )';
                    print_r($nepali_conversion);
                    break;
            }
        }
    }
        ...
        if (isset($_POST['cnv'])) {
            ...
            switch ($_POST['cnv']) {
                case 'to_english':
                    ...
                    break;
                case 'to_nepali':
                    die($_POST['yy']." ".$_POST['mm']." ".$_POST['dd']);
                    //die('y');
                    $cal = $this->eng_to_nep($_POST['yy'], $_POST['mm'], $_POST['dd']);
                    //print_r($cal);
                    $nepali_conversion = $cal['year'] . '-' . $cal['month'] . '-' . $cal['date'] . ' ( ' . $cal['nmonth'] . '/' . $cal['day'] . ' )';
